"""
LQR with distractor observations: two-layer (i.e., overparameterized) policy. 
"""
...
def main(raw_args=None):
	...
	##############################################################
	# Train two-layer policy using gradient descent
	##############################################################

	# Define two-layer policy
	width = 10*nu
	K1 = torch.nn.Parameter(0.001*torch.randn(width, nx+nyp))
	K2 = torch.nn.Parameter(0.001*torch.randn(nu, width))

	# Optimizer
	optimizer = torch.optim.Adam([K1, K2], lr=0.001) 

	# Gradient descent on combined cost of lqr1 and lqr2
	num_iters = 5000
	for iter in range(0, num_iters):
		...
		for i in range(num_domains):
			cost += lqr_all[i].closed_form_costs(K)
	...
	print(" ")
	print("Costs on test LQR system (multi-layer)")
